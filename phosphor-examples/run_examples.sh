...
function run_example {
  ...
  if [ -d $jre_inst ]
    then
      ...
      $jre_inst/bin/java -Xbootclasspath/a:$phosphor_jar -javaagent:$phosphor_jar=taintSources=src/main/resources/taint-sources,taintSinks=src/main/resources/taint-sinks -cp $examples_jar\
      -ea $class $rest
    else
 ...
 function inst_jar {
  ...
  #  java -jar $phosphor_jar $src $dest_folder
   java -jar $phosphor_jar -controlTrack $src $dest_folder
...
JRE_INST=${1}/jre-inst
PHOSPHOR_JAR=$(find $1 -maxdepth 1 -iname "Phosphor-[0-9]*SNAPSHOT.jar")
...
run_example $JRE_INST $PHOSPHOR_JAR $EXAMPLES_JAR com.josecambronero.IntegerTagExamples
run_example $JRE_INST $PHOSPHOR_JAR $EXAMPLES_JAR com.josecambronero.ObjectTagExamples
run_example $JRE_INST $PHOSPHOR_JAR $EXAMPLES_JAR com.josecambronero.ImplicitFlowsExamples
# only runnning with integer tags, but the idea is the same for other cases
run_example $JRE_INST $PHOSPHOR_JAR $INST_EXAMPLES_JAR com.josecambronero.AutoExample
...
run_example $JRE_INST $PHOSPHOR_JAR $EXAMPLES_JAR com.josecambronero.AutoExample\
